...
from bamboo._exception import BambooException, BambooInputException, BambooOutputException, BambooTransformationException
...
def _validate_input_output_series(
    ...
    try:
        input_args = {str(key): value for key, value in input_series.to_dict().items()}
        transformation_input = InputType(**input_args)
    except Exception as ex:
        raise BambooInputException(f"Creating data object of type {InputType} with input {input_series} failed.") from ex
    ...
    try:
        ...
        raise BambooTransformationException("Data transformation raised an exception.") from ex

    # Parse output
    if not isinstance(transformation_output, OutputType):
        raise BambooOutputException(
            f"Expected transformation of type {OutputType}, " f"but received {type(transformation_output)}"
        )
